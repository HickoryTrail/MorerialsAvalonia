...
该包目前仅支持 Windows。本项目使用纯 GPU 渲染路线，不支持 CPU Fallback，因此不支持的平台会通过 `MaterialHost.Diagnostics` 报告初始化错误。
...
`tests/MorerialsAvalonia.PackageSmoke` 必须在包生成后从本地 NuGet 源还原，用于验证真实 `.nupkg` 的 AXAML 编译和嵌入着色器加载。CI 会自动执行这一步。
...
- 为每个 `*DemoGallery` 项目生成 `win-x64` zip。
- 创建 GitHub Release，并同时上传 `.nupkg` 与所有 DemoGallery zip。

发布说明放在 `docs/CHANGELOG/vX.Y.Z.md`。

仓库需要一个名为 `release` 的受保护 Environment，并在其中配置 NuGet Trusted Publishing 所需的 `NUGET_USERNAME` 变量。GitHub Actions 的 `GITHUB_TOKEN` 由工作流权限用于 GitHub Packages 和 GitHub Release。
